...
from database import CLIENTS_FILE, ORDERS_FILE, PRODUCTS_FILE, money, next_order_number, now_text, read_table, save_table, to_excel_bytes
...
def show_new_order() -> None:
    ...
    c1, c2 = st.columns(2)
    with c1:
        client_list = clients["cliente"].astype(str).tolist() if "cliente" in clients.columns and len(clients) else ["CLIENTE PADRÃO"]
    ...
    for _, row in filtered.head(8).iterrows():
        ...
        with a:
            st.markdown(f"""
            <div class='sugestao'>
                <span class='codigo'>{row['codigo']}</span> - {row['produto']} | {money(row['preco'])}<br>
                <small>Fornecedor: {row.get('fornecedor', '')}</small>
            </div>
            """, unsafe_allow_html=True)
        with b:
            if st.button("Selecionar", key=f"select_{row['codigo']}"):
                st.session_state.produto_selecionado = row.to_dict()
                st.rerun()

    if st.session_state.get("produto_selecionado"):
        product = st.session_state.produto_selecionado
        st.markdown("### Produto selecionado")
        st.markdown(f"<div class='card'><b>{product['codigo']} - {product['produto']}</b><br>Fornecedor: {product.get('fornecedor', '')}</div>", unsafe_allow_html=True)
        ...
        with q3:
            ...
            st.text_input("Preço", value=money(price), disabled=True)

        subtotal = price * quantity
        total = subtotal - (subtotal * discount / 100)
        st.markdown(f"### Total do item: {money(total)}")
...
def show_orders() -> None:
    ...
    st.download_button("⬇️ Baixar pedidos em Excel", to_excel_bytes(orders), file_name="pedidos_tigrao.xlsx")
    ...
    if is_admin():
        ...
        if len(order_data):
            st.warning(f"Você está prestes a excluir o pedido nº {selected} do cliente {order_data['cliente'].iloc[0]}, total {money(order_data['total'].sum())}.")
